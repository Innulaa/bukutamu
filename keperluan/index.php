...
$where = $search ? "WHERE nama_keperluan LIKE '%$search%'" : '';
$total_query = mysqli_query($conn, "SELECT COUNT(*) as total FROM keperluan $where");
$total_data = mysqli_fetch_assoc($total_query)['total'];
$total_pages = ceil($total_data / $limit);

$data = mysqli_query($conn, "SELECT * FROM keperluan $where ORDER BY id DESC LIMIT $limit OFFSET $offset");
...
<header class="bg-white py-4 px-6 shadow flex justify-between items-center fixed top-0 left-64 right-0 z-40">
  <h1 class="text-xl font-bold text-orange-600">Data Keperluan</h1>
  ...
  <div class="bg-white p-6 rounded-lg shadow">
    ...
    <div class="flex justify-end mb-2">
      <form method="GET" class="flex gap-2">
        <input type="text" name="search" value="<?= htmlspecialchars($search) ?>" placeholder="Cari keperluan..." class="border border-gray-300 rounded px-3 py-1 text-sm">
      ...
      <table class="w-full text-sm text-left border-t">
        <thead class="table-header">
          <tr>
            ...
            <th class="px-4 py-2">Nama Keperluan</th>
            ...
            <?php $no = $offset + 1; while ($row = mysqli_fetch_assoc($data)): ?>
              <tr class="border-b table-row">
                ...
                <td class="px-4 py-2 text-center">
                  <button onclick="openEditModal(<?= $row['id'] ?>, '<?= addslashes($row['nama_keperluan']) ?>')" class="text-orange-600 hover:text-orange-800 mx-1" title="Edit">
...
<div id="addModal" class="fixed inset-0 bg-black bg-opacity-40 hidden items-center justify-center z-50">
  <div class="bg-white w-96 rounded-lg shadow p-6">
    <h2 class="text-lg font-semibold mb-4 text-orange-700">Tambah Keperluan</h2>
    <form method="POST" action="tambah.php">
      <label class="block mb-2 text-sm font-medium">Nama Keperluan</label>
      <input type="text" name="nama_keperluan" class="w-full border px-3 py-2 rounded mb-4" required>
...
<div id="editModal" class="fixed inset-0 bg-black bg-opacity-40 hidden items-center justify-center z-50">
  <div class="bg-white w-96 rounded-lg shadow p-6">
    <h2 class="text-lg font-semibold mb-4 text-orange-700">Edit Keperluan</h2>
    <form method="POST" action="edit.php">
      <input type="hidden" name="id" id="editId">
      <label class="block mb-2 text-sm font-medium">Nama Keperluan</label>
      <input type="text" name="nama_keperluan" id="editNama" class="w-full border px-3 py-2 rounded mb-4" required>
      <div class="flex justify-end gap-2">
